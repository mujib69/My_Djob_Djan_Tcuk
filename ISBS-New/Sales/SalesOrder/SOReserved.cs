...
{
    public partial class SOReserved : MetroFramework.Forms.MetroForm
    {
        /**********SQL*********/
        ...
        string[] tableCols = new string[] { "No", "RefTransType", "RefTransId", "RefTrans_SeqNo", "RefTrans2Id", "RefTrans2_SeqNo", "GroupID", "SubGroup1ID", "SubGroup2ID", "ItemID", "FullItemId", "ItemName", "InventSiteId", "Available_UoM", "Available_For_Sale_UoM", "Available_For_Sale_Reserved_UoM", "Lock_Qty", "Unit", "Lock_Qty_Alt", "Unit_Alt", "Ratio" };
        /*********datagridview cols name*********/
        ...
        public List<string> SOID = new List<string>();
        public List<int> SO_SeqNo = new List<int>();
        public List<decimal> SO_Qty = new List<decimal>();
        //public List<int> rowNum = new List<int>();
        ...
        private void SOReserved_Load(object sender, EventArgs e)
        {
            GetDataHeader();
            gv1Format();
        }
        ...
        private void GetDataHeader()
        {
            ...
            Query = "select d.SalesOrderNo, d.SeqNo, d.SA_SQ_Id, d.SA_SQ_SeqNo, c.GroupID, c.SubGroup1ID, c.SubGroup2ID, c.ItemID, a.FullItemId, a.ItemName, a.InventSiteId, a.Available_UoM, a.Available_For_Sale_UoM, a.Available_For_Sale_Reserved_UoM, b.Ratio, c.UoM, c.UoMAlt from Invent_OnHand_Qty a left join InventConversion b on a.FullItemId = b.FullItemID left join InventTable c on c.FullItemID = a.FullItemID left join SalesOrderD d on a.FullItemId = d.FullItemID where a.FullItemID in (select FullItemID from SalesOrderD where SalesOrderNo = '" + SOID[0] + "' and Deleted = 'N') and d.SalesOrderNo = '" + SOID[0] + "' and d.Deleted = 'N'";
            ...
            while (Dr.Read())
            {
                ...
                dataGridView1.Rows[dataGridView1.RowCount - 1].Cells["RefTransType"].Value = "SALES ORDER";
                dataGridView1.Rows[dataGridView1.RowCount - 1].Cells["RefTransId"].Value = Dr["SalesOrderNo"];
                ...
                Query = "select Lock_Qty, Lock_Qty_Alt from InventLockTable where RefTransId = '" + Dr["SalesOrderNo"] + "' and RefTrans_SeqNo = '" + Dr["SeqNo"] + "' and RefTrans2Id = '" + Dr["SA_SQ_Id"] + "' and RefTrans2_SeqNo = '" + RefTrans2_SeqNo + "' and FullItemId = '" + Dr["FullItemId"] + "' and SiteId = '" + Dr["InventSiteId"] + "'";
                Cmd = new SqlCommand(Query, Conn);
        ...
        {
            //dataGridView1.Columns["RefTrans_SeqNo"].HeaderText = "SO Item No";
        ...
        private void SOReserved_Leave(object sender, EventArgs e)
        {
            SOID.Clear();
            SO_SeqNo.Clear();
            SO_Qty.Clear();
        ...
        private char Validation()
        {
            ...
            for (int i = 0; i < dataGridView1.RowCount; i++)
            {
                ...
                if (!(dataGridView1.Rows[i].Cells["Lock_Qty"].Value == null || dataGridView1.Rows[i].Cells["Lock_Qty"].Value == String.Empty || dataGridView1.Rows[i].Cells["Lock_Qty"].Value == (object)DBNull.Value))
                    populateQty = Convert.ToDecimal(dataGridView1.Rows[i].Cells["Lock_Qty"].Value);
                ...
                if (!(dataGridView1.Rows[i].Cells["RefTransId"].Value.ToString() == id && Convert.ToInt32(dataGridView1.Rows[i].Cells["RefTrans_SeqNo"].Value) == seqNo))
                {
                    ...
                    {
                        if (dataGridView1.Rows[i].Cells["RefTransId"].Value.ToString() == dataGridView1.Rows[j].Cells["RefTransId"].Value.ToString() && Convert.ToInt32(dataGridView1.Rows[i].Cells["RefTrans_SeqNo"].Value) == Convert.ToInt32(dataGridView1.Rows[j].Cells["RefTrans_SeqNo"].Value))
                        {
                            if (i != j)
                            {
                    ...
                    if (populateQty > SO_Qty[x])
                        msg += "Total Quantity for item " + dataGridView1.Rows[i].Cells["FullItemId"].Value.ToString() + " (" + dataGridView1.Rows[i].Cells["ItemName"].Value.ToString() + ") cannot more than " + SO_Qty[x] + "!\r\n";
                        //msg += "Total Quantity of SO Item No " + dataGridView1.Rows[i].Cells["RefTrans_SeqNo"].Value.ToString() + " (" + dataGridView1.Rows[i].Cells["FullItemId"].Value.ToString() + " " + dataGridView1.Rows[i].Cells["ItemName"].Value.ToString() + ") cannot more than " + SO_Qty[x] + "!\r\n";
                    x++;
                }
            }
            ...
            try
            {
                using (scope = new TransactionScope())
                {
                    ...
                    for (int i = 0; i < dataGridView1.RowCount; i++)
                    {
                        ...
                        Query = "select d.RefTransId, d.RefTrans_SeqNo, d.Lock_Qty, d.Lock_Qty_Alt, a.FullItemId, a.ItemName, a.InventSiteId, a.Available_UoM, a.Available_For_Sale_UoM, a.Available_For_Sale_Reserved_UoM, b.Ratio, c.UoM, c.UoMAlt from Invent_OnHand_Qty a left join InventConversion b on a.FullItemId = b.FullItemID left join InventTable c on c.FullItemID = a.FullItemID left join InventLockTable d on d.FullItemId = a.FullItemId and d.SiteId = a.InventSiteId where a.FullItemID in (select FullItemID from SalesOrderD where SalesOrderNo = '" + SOID[0] + "' and Deleted = 'N') and d.RefTransId = '" + SOID[0] + "' and d.RefTrans_SeqNo = '" + RefTrans_SeqNo + "' and d.RefTrans2Id = '" + RefTrans2Id + "' and d.RefTrans2_SeqNo = '" + RefTrans2_SeqNo + "' and d.SiteId = '" + SiteId + "'";
                        Cmd = new SqlCommand(Query, Conn);
                        Dr = Cmd.ExecuteReader();
                        if (Dr.HasRows)
                        {
                        ...
                        else if (Lock_Qty != 0)
                        {
                            ...
                            insertInventTrans(GroupId, SubGroup1Id, SubGroup2Id, ItemId, FullItemId, ItemName, SiteId, RefTransId, RefTrans_SeqNo, Lock_Qty * -1, Lock_Qty * -1 * Ratio, Lock_Qty * price * -1, Lock_Qty, Lock_Qty * Ratio, Lock_Qty * price);
                        }
                        Dr.Close();
                    }
                    Conn.Close();
        ...
        private void insertInventTrans(string GroupId, string SubGroupId, string SubGroup2Id, string ItemId, string FullItemId, string ItemName, string InventSiteId, string TransId, int SeqNo, decimal Available_For_Sale_UoM, decimal Available_For_Sale_Alt, decimal Available_For_Sale_Amount, decimal Available_For_Sale_Reserved_UoM, decimal Available_For_Sale_Reserved_Alt, decimal Available_For_Sale_Reserved_Amount)
        {
            ...
            string AccountId = "";
            string AccountName = "";
            Query = "select a.OrderDate, b.SA_SQ_Id, b.SA_SQ_SeqNo, a.CustId, a.CustName from SalesOrderH a left join SalesOrderD b on a.SalesOrderNo = b.SalesOrderNo where b.SalesOrderNo = '" + TransId + "' and b.SeqNo = '" + SeqNo + "'";
            Cmd = new SqlCommand(Query, Conn);
            SqlDataReader Dr2 = Cmd.ExecuteReader();
            while (Dr2.Read())
            {
                ...
                if (Dr2["SA_SQ_Id"] != (object)DBNull.Value)
                {
                    if (Dr2["SA_SQ_Id"].ToString().Split('/')[0] == "SA")
                    {
                        Query = "select OrderDate from SalesAgreementH where SalesAgreementNo = '" + Dr2["SA_SQ_Id"] + "'";
                        Cmd = new SqlCommand(Query, Conn);
                        Ref_TransDate = Convert.ToDateTime(Cmd.ExecuteScalar());
                    }
                    else if (Dr2["SA_SQ_Id"].ToString().Split('/')[0] == "SQ")
                    {
                        Query = "select OrderDate from SalesQuotationH where SalesQuotationNo = '" + Dr2["SA_SQ_Id"] + "'";
                        Cmd = new SqlCommand(Query, Conn);
                        Ref_TransDate = Convert.ToDateTime(Cmd.ExecuteScalar());
                    }
                }
            ...
            Cmd.Parameters.AddWithValue("@Ref_TransId", Ref_TransId == String.Empty ? (object)DBNull.Value : Ref_TransId);
            Cmd.Parameters.AddWithValue("@Ref_TransDate", Ref_TransId == String.Empty ? (object)DBNull.Value : Ref_TransDate);
            Cmd.Parameters.AddWithValue("@Ref_Trans_SeqNo", Ref_TransId == String.Empty ? (object)DBNull.Value : Ref_Trans_SeqNo);
