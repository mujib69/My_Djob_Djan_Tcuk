        ...
        public static void actionRequestCancelDO(string Action, string DONo)
        {
            ...
            if (checkSI == "0") //kalau belum ada SI
            {
                if (Action == "Approve")
                {
                    ...
                    Query += "[ApproveCancelSMBy] = '" + ControlMgr.UserId + "' ";
            ...
            else //kalau sudah tarik SI *******************************************
            {
                if (Action == "Approve")
                {
                    ...
                    if (ControlMgr.GroupName == "Expedisi Manager")
                    {
                        /*jika exp man, yes:
                         * maka status do cancelled. - done
                         * update remaining SO, - done, need check 
                        ...
                        Query += "[ApproveCancelEXPBy] = '" + ControlMgr.UserId + "' ";
                        Query += "WHERE DeliveryOrderId = '" + DONo + "'";
                        using (Cmd = new SqlCommand(Query, ConnectionString.GetConnection()))
                            Cmd.ExecuteNonQuery();
        ...
        private static void updateRemainingSO(string DONo)
        {
            ...
            for (int i = 1; i < (totalDetail + 1); i++)
            {
                ...
                while (Dr.Read())
                {
                    ...
                    //update DO RemainingQty
                    Query = "UPDATE [DeliveryOrderD] SET [RemainingQty] = [RemainingQty] - '" + DORemainingQty + "' ";
                    Query += "WHERE [DeliveryOrderId] = '" + DONo + "' AND [SeqNo] = '" + i + "'";
                    using (Cmd = new SqlCommand(Query, ConnectionString.GetConnection()))
                        Cmd.ExecuteNonQuery();
                }
            }
