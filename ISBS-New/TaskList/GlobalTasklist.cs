...
{
    public partial class GlobalTasklist : MetroFramework.Forms.MetroForm
    {
        ...
        private void GlobalTasklist_Load(object sender, EventArgs e)
        {
            if (FormName == "DOInq")
        ...
        public void RefreshGrid()
        {
            ...
            if (FormName == "InquiryNotaResize" || FormName == "Payment Mode" || FormName == "Term of Payment" || FormName == "Delivery Method" || FormName == "Sales" || FormName == "Counter" || FormName == "Bank Group" || FormName == "Bank" || FormName == "Currency" || FormName == "Exchange Rate")
            {
                if (crit == null)
                {
                    Query += ") a ";
            ...
            dataGridView1.Columns["Send Email"].Visible = false;
        }
        ...
        private void CallForm(string tmpMode)
        {
            ...
            if (FormName == "DOInq")
            {
                ISBS_New.Sales.DeliveryOrder.DOHeader F = new ISBS_New.Sales.DeliveryOrder.DOHeader();
            ...
            {
                ISBS_New.Sales.BBK.BBKHeader F;
                if (ReferenceType == "NOTA TRANSFER")
                {
                    F = new ISBS_New.Sales.BBK.BBKHeader(ReferenceType);
                }
                else
                {
                    F = new ISBS_New.Sales.BBK.BBKHeader();
                }
            ...
            {
                ISBS_New.Sales.SalesAgreement.SAHeader F = new ISBS_New.Sales.SalesAgreement.SAHeader();
                if (F.PermissionAccess(ControlMgr.View) > 0)
                {
                    F.SetParent2(this);
                    F.SetMode(tmpMode, temp);
            ...
            {
                ISBS_New.Sales.SalesOrder.SOHeader F = new ISBS_New.Sales.SalesOrder.SOHeader();
                if (F.PermissionAccess(ControlMgr.View) > 0)
                {
                    F.SetParent2(this);
                    F.SetMode(tmpMode, temp);
            ...
            {
                ISBS_New.AccountsReceivable.ReceiptVoucher.HeaderReceiptVoucher F = new ISBS_New.AccountsReceivable.ReceiptVoucher.HeaderReceiptVoucher();
                if (F.PermissionAccess(ControlMgr.View) > 0)
                {
                    F.SetParent2(this);
                    F.SetMode(tmpMode, temp);
        ...
        {
            ISBS_New.Sales.BBK.UbahReceiptDate F = new ISBS_New.Sales.BBK.UbahReceiptDate();
