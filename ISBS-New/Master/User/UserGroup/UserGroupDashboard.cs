...
{
    public partial class UserGroupDashboard : Form
    {
        //SQL Function
        ...
        public void RefreshGrid()
        {
            try
            {
                ...
                Query = "Select No, UserId, UPPER(UserId) UserId, UPPER(GroupName) GroupName  From (SELECT ROW_NUMBER() OVER (ORDER BY [UserId]) No, [UserId], [UserId],STUFF((SELECT ';' + [GroupName] FROM [User].[UserGroup] a WHERE (a.[UserId] = b.[UserId]) and ([Status] = 1) FOR XML PATH ('')), 1, 1, '') GroupName FROM [User].[UserGroup] b ";
                if (txtSearch.Text.Trim() != "")
                    Query += "where " + Filter;
                Query += "group by [UserId],[UserId] ) a Where No Between " + Limit1 + " and " + Limit2 + " ;";
                ...
                Query = "Select Count(" + IdNamePK + ") From (Select No, UserId, UPPER(UserId) UserId, UPPER(GroupName) GroupName  From (SELECT ROW_NUMBER() OVER (ORDER BY [UserId]) No, [UserId], [UserId],STUFF((SELECT ';' + [GroupName] FROM [User].[UserGroup] a WHERE (a.[UserId] = b.[UserId]) and ([Status] = 1) FOR XML PATH ('')), 1, 1, '') GroupName FROM [User].[UserGroup] b ";
                if (txtSearch.Text.Trim() != "")
                    Query += "where " + Filter;
                Query += "group by [UserId],[UserId] ) a ) b ; ";
        ...
        private void SelectData()
        {
            try
            {
                if (dgvSearch.RowCount >= 1)
                {
                    ...
                    Master.User.UserGroup.UserGroupForm F = new Master.User.UserGroup.UserGroupForm();
                    F.Show();
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Index >= 0)
                {
                    ...
                    {
                        Conn = ConnectionString.GetConnection();
                        Query = "Delete from [" + SchemaName + "].[" + TableName + "] where UserId = '" + dgvSearch["UserId", Index].Value.ToString() + "'";
        ...
        private void UserGroupDashboard_FormClosed(object sender, FormClosedEventArgs e)
        {
            timerRefresh.Stop();
        }
