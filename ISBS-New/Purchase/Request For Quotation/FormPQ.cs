...
namespace ISBS_New.Purchase.PurchaseQuotation
{
    public partial class FormPQ : MetroFramework.Forms.MetroForm
    {
        ...
        public void AddDataGridDetail(List<string> SeqNum)
        {
            ...
            for (int i = 0; i < SeqNum.Count; i++)
            {
                ...
                while (Dr.Read())
                {
                    ...
                    Query = "Select [Uom], [UomAlt] From dbo.[InventTable] where FullItemId = '" + Dr[7].ToString() + "' ";
                    Cmd = new SqlCommand(Query, Conn);
                    Dr = Cmd.ExecuteReader();
                    DataGridViewComboBoxCell combo1 = new DataGridViewComboBoxCell();
                    while (Dr.Read())
                    {
                        combo1.Items.Add(Dr[0].ToString());
                        combo1.Items.Add(Dr[1].ToString());
                    }
                    ...
                    while (Dr.Read())
                    {
                        combo2.Items.Add(Dr[0].ToString());
                    }
                    dgvPqDetails.Rows[(dgvPqDetails.Rows.Count - 1)].Cells[11] = combo2;
