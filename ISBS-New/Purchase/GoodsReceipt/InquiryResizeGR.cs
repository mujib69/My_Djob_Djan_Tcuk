...
{
    public partial class InquiryResizeGR : MetroFramework.Forms.MetroForm
    {
        ...
        public InquiryResizeGR()
        {
            InitializeComponent();
        }

        private void InquiryResizeGR_Load(object sender, EventArgs e)
        {
            addCmbCrit();
            ModeLoad();
        }
        ...
        public void RefreshGrid()
        {
            //Menampilkan data
            Conn = ConnectionString.GetConnection();
            Query = "Select * From ( Select ROW_NUMBER() OVER (ORDER BY [TransId] desc) [No], [TransId], [TransDate], [CreatedDate], [CreatedBy], [UpdatedDate], [UpdatedBy], [Posted], [RecId] From [dbo].[ResizeH] ";

            if (crit == null)
                Query += "where [Posted] != '2' ) a ";
            else if (crit.Equals("All"))
            {
                Query += "where [TransId] like @txtSearch and [Posted] != '2' ) a ";
            }
            ...
            dataGridView1.Columns["CreatedDate"].DefaultCellStyle.Format = "dd/MM/yyyy hh:mm tt";
            dataGridView1.Columns["UpdatedDate"].DefaultCellStyle.Format = "dd/MM/yyyy hh:mm tt";
            Conn.Close();

            //Mengambil nilai total paging
            Conn = ConnectionString.GetConnection();
            Query = "Select Count([TransId]) From ( Select [TransId] From [dbo].[ResizeH] ";
            if (crit == null)
                Query += "where [Posted] != '2' ) a;";
            else if (crit.Equals("All"))
            {
            ...
            if (this.PermissionAccess(ControlMgr.Delete) > 0)
            {
                ...
                else
                {
                    ...
                    if (dialogResult == DialogResult.Yes)
                    {
                        ...
                        foreach (DataGridViewRow r in dataGridView1.SelectedRows)
                        {
                            Query = "update [dbo].[ResizeH] set [Posted] = '2' where [TransId] = '" + dataGridView1.Rows[r.Index].Cells["TransId"].Value.ToString() + "'; ";
                            Cmd = new SqlCommand(Query, Conn);
                            /*int result = */
                            Cmd.ExecuteNonQuery();
                        }
