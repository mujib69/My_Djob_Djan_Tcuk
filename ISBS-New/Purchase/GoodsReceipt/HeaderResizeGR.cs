...
{
    public partial class HeaderResizeGR : MetroFramework.Forms.MetroForm
    {
        ...
        InquiryResizeGR Parent = new InquiryResizeGR();
        public void SetParent(InquiryResizeGR F)
        {
            Parent = F;
        }
        ...
        public HeaderResizeGR()
        {
            InitializeComponent();
        }

        private void ResizeGR_Load(object sender, EventArgs e)
        {
            if (Mode == "BeforeEdit" || Mode == "Edit")
            {
                Conn = ConnectionString.GetConnection();
                Cmd = new SqlCommand("select b.[RefTransId] from [dbo].[ResizeH] as a left join [dbo].[Resize_Dtl] as b on a.[TransId] = b.[TransId] where a.[TransId] = '" + tbxResizeID.Text + "'; ", Conn);
        ...
        public string GenerateID()
        {
            Conn = ConnectionString.GetConnection();
            count = 0;
            Cmd = new SqlCommand("Select count(*) from [dbo].[ResizeH]", Conn);
            count = (Int32)Cmd.ExecuteScalar();
            if (count == 0)
                count++;
            else
            {
                Cmd = new SqlCommand("SELECT TOP 1 [TransId] FROM [dbo].[ResizeH] order by [CreatedDate] desc", Conn);
        ...
        private void RefreshGrid()
        {
            ...
            if (Mode == "New")
            {
                Cmd = new SqlCommand("select top 1 RefTransId from Resize_Dtl where RefTransId = '" + tbxGRID.Text + "'", Conn);
                Dr = Cmd.ExecuteReader();
                if (Dr.HasRows)
                {
                    Query = " Select b.[GroupId] ,b.[SubGroup1Id] ,b.[SubGroup2Id] ,b.[ItemId] , b.[Qty_Actual], b.[Unit], b.[GoodsReceivedSeqNo], b.[FullItemId], b.[ItemName] from [dbo].[GoodsReceivedH] as a left join [dbo].[GoodsReceivedD] as b  on a.[GoodsReceivedId] = b.[GoodsReceivedId] left join Resize_Dtl as c  on a.GoodsReceivedId = c.RefTransId left join ResizeH as d on c.TransId = d.TransId where a.[GoodsReceivedId] = '" + tbxGRID.Text + "' and d.Posted != '02' and b.FullItemId != c.OriginalFullItemId";
                }
                else
                {
            ...
            {
                Query = "Select b.[GroupId] ,b.[SubGroup1Id] ,b.[SubGroup2Id] ,b.[ItemId] , b.[Qty], b.[Unit], b.[ParentSeqNo], b.[FullItemId], b.[ItemName], b.OriginalFullItemId, b.OriginalItemName from [dbo].[ResizeH] as a left join [dbo].[Resize_Dtl] as b on a.[TransId] = b.[TransId] where a.[TransId] = '" + tbxResizeID.Text + "' and [OriginalItemId] is not null; ";
                Cmd = new SqlCommand(Query, Conn);
                Dr = Cmd.ExecuteReader();
                while (Dr.Read())
                {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (Validation() != 'X')
            {
                ...
                if (Mode != "Edit")
                {
                    ...
                    Query = "insert into [dbo].[ResizeH] ([TransDate] ,[TransId] ,[CreatedDate] ,[CreatedBy],[Posted])values ('" + dateTimePicker1.Value + "', '" + tbxResizeID.Text + "', getdate(), '" + ControlMgr.GroupName + "', 1); ";
                    ...
                    for (int i = 0; i < dataGridView1.Rows.Count; i++)
                    {
                        if (dataGridView1.Rows[i].Cells["ToFullItemId"].Value != null && dataGridView1.Rows[i].Cells["ToItemName"].Value != null)
                        {
                            seqNo += 1;
                            Query += "insert into [dbo].[Resize_Dtl] ( [TransId],[SeqNo],[GroupId],[SubGroup1Id],[SubGroup2Id],[ItemId],[FullItemId],[ItemName],[InventSiteIdIssue],[InventSiteIdReceive],[Qty],[Unit],[RefTransId],[RefSeqNo],[OriginalGroupId] ,[OriginalSubGroup1Id] ,[OriginalSubGroup2Id] ,[OriginalItemId] ,[OriginalFullItemId], [OriginalItemName], [ParentSeqNo] ,[CreatedDate] ,[CreatedBy] ) ";
                            Query += "values ('" + tbxResizeID.Text + "', '" + seqNo + "', '" + dataGridView1.Rows[i].Cells["ToFullItemId"].Value.ToString().Split('.')[0] + "', '" + dataGridView1.Rows[i].Cells["ToFullItemId"].Value.ToString().Split('.')[1] + "', '" + dataGridView1.Rows[i].Cells["ToFullItemId"].Value.ToString().Split('.')[2] + "', '" + dataGridView1.Rows[i].Cells["ToFullItemId"].Value.ToString().Split('.')[3] + "', '" + dataGridView1.Rows[i].Cells["ToFullItemId"].Value.ToString() + "', '" + dataGridView1.Rows[i].Cells["ToItemName"].Value.ToString() + "', NULL, NULL, '" + dataGridView1.Rows[i].Cells["Qty_Actual"].Value.ToString() + "', '" + dataGridView1.Rows[i].Cells["Unit"].Value.ToString() + "', '" + tbxGRID.Text + "', NULL, '" + dataGridView1.Rows[i].Cells["GroupId"].Value.ToString() + "', '" + dataGridView1.Rows[i].Cells["SubGroup1Id"].Value.ToString() + "', '" + dataGridView1.Rows[i].Cells["SubGroup2Id"].Value.ToString() + "', '" + dataGridView1.Rows[i].Cells["ItemId"].Value.ToString() + "', '" + dataGridView1.Rows[i].Cells["FullItemId"].Value.ToString() + "', '" + dataGridView1.Rows[i].Cells["ItemName"].Value.ToString() + "', '" + dataGridView1.Rows[i].Cells["GoodsReceivedSeqNo"].Value.ToString() + "', getdate(), '" + ControlMgr.GroupName + "'); ";
                        }
                    }
                }
                else if (Mode == "Edit")
                {
                    Query = "update [dbo].[ResizeH] set [UpdatedDate] = getdate(), [UpdatedBy] = '" + ControlMgr.GroupName + "' where [TransId] = '" + tbxResizeID.Text + "'; ";
                    for (int i = 0; i < dataGridView1.RowCount; i++)
                    {
                        Query += "update [dbo].[Resize_Dtl] set [FullItemId] = '" + dataGridView1.Rows[i].Cells["ToFullItemId"].Value.ToString() + "', [ItemName] = '" + dataGridView1.Rows[i].Cells["ToItemName"].Value.ToString() + "', [UpdatedDate] = getdate(), [UpdatedBy] = '" + ControlMgr.GroupName + "' where [ParentSeqNo] = '" + dataGridView1.Rows[i].Cells["GoodsReceivedSeqNo"].Value.ToString() + "'; ";
                    }
                }
                Cmd = new SqlCommand(Query, Conn);
        ...
        private void HeaderResizeGR_FormClosed(object sender, FormClosedEventArgs e)
        {
            GRID = null;
        }
        ...
        {
            PopUpGR f = new PopUpGR();
            f.ShowDialog();
            tbxGRID.Text = PopUpGR.grID;
            RefreshGrid();
        }
    }
}
