...
{
    public partial class InquiryGoodsReceipt : MetroFramework.Forms.MetroForm
    {
        //private string vTransType;
        private List<string> vTransType;
        ...
        List<GRHeaderV2> ListHeaderGoodsReceipt = new List<GRHeaderV2>();
        ...
        public InquiryGoodsReceipt(List<string> _vTransType)
        {
            InitializeComponent();
            vTransType = _vTransType;
        }

        public InquiryGoodsReceipt(string _vTransType2)
        {
            InitializeComponent();
            vTransType = new List<string>();
            vTransType.Add(_vTransType2);
        }

        private void InquiryGoodsReceipt_Load(object sender, EventArgs e)
        {
            addCmbCrit();
            ModeLoad();
        ...
        public void RefreshGrid()
        {
            ...
            Query += "CreatedBy AS 'Created By',CreatedDate AS 'Created Date',UpdatedBy AS 'Updated By', CASE WHEN UpdatedBy = '' then null else UpdatedDate END 'Updated Date' ";
            Query += "FROM GoodsReceivedH AS a LEFT JOIN TransStatusTable AS b ON b.StatusCode = a.GoodsReceivedStatus ";
            Query += "WHERE b.TransCode = 'GR' AND a.[GoodsReceivedStatus] != '04' ";
            Query += " AND a.RefTransType IN ( ";

            for (int i = 0; i < vTransType.Count; i++)
            {
                if (i == (vTransType.Count - 1))
                {
                    Query += " '" + vTransType[i] + "' )";
                }
                else
            ...
            {
                Query += "and (CONVERT(VARCHAR(10),CreatedDate,120) >= '" + dtFrom.Value.Date.ToString("yyyy-MM-dd") + "' And CONVERT(VARCHAR(10),CreatedDate,120) <= '" + dtTo.Value.Date.ToString("yyyy-MM-dd") + "') And ([GoodsReceivedStatus] != '04')) a ";
            }
            ...
            dgvPR.Columns["Reference Date"].DefaultCellStyle.Format = "dd/MM/yyyy";
            dgvPR.Columns["Created Date"].DefaultCellStyle.Format = "dd/MM/yyyy hh:mm tt";
            dgvPR.Columns["Updated Date"].DefaultCellStyle.Format = "dd/MM/yyyy hh:mm tt";
            Conn.Close();
            ...
            Query += " AND b.TransCode = 'GR' ";
            if (crit == null)
                Query += "AND [GoodsReceivedStatus] != '04') a;";
            else if (crit.Equals("All"))
            {
                Query += "AND ([GoodsReceivedId] like @txtSearch or [GoodsReceivedStatus] like @txtSearch or [SJNumber] like @txtSearch) and [GoodsReceivedStatus] != '04') a ";
            }
            else if (crit.Contains("Date"))
            {
                Query += "and (CONVERT(VARCHAR(10)," + crit + ",120) >= '" + dtFrom.Value.Date.ToString("yyyy-MM-dd") + "' And CONVERT(VARCHAR(10)," + crit + ",120) <= '" + dtTo.Value.Date.ToString("yyyy-MM-dd") + "') And ([GoodsReceivedStatus] != '04')) a ";
            }
            else if (crit.Equals("GoodsReceivedStatus"))
            {
                Query += " AND b.[Deskripsi] like @txtSearch and [GoodsReceivedStatus] != '04') a ";
            }
            else
            {
                Query += "and (" + crit + " like @txtSearch ) and [GoodsReceivedStatus] != '04') a ";
            }
        ...
        private void btnNew_Click(object sender, EventArgs e)
        {
            ...
            if (this.PermissionAccess(ControlMgr.New) > 0)
            {
                ...
                //{
                    Purchase.GoodsReceipt.GRHeaderV2 f = new Purchase.GoodsReceipt.GRHeaderV2(vTransType[0]);
                    //Purchase.GoodsReceipt.HeaderGoodsReceipt f = new Purchase.GoodsReceipt.HeaderGoodsReceipt();
                    f.SetMode("New", "");
                    f.SetParent(this);
                    f.Show();
                    RefreshGrid();
        ...
        private void SelectPR()
        {
            ...
            string refType = dgvPR.Rows[dgvPR.CurrentCell.RowIndex].Cells["Reference"].Value.ToString();
            if (refType.Contains("RO"))
            {
                refType = "Receipt Order"; 
            }
            else if (refType.Contains("NRB"))
            {
                refType = "Nota Retur Beli";
            }
            else if (refType.Contains("NRJ"))
            {
                refType = "Nota Retur Jual";
            }
            else if (refType.Contains("NT"))
            {
                refType = "Nota Transfer";
            }
            GRHeaderV2 f = new GRHeaderV2(refType);
        ...
        private void dgvPR_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex != -1 && e.RowIndex > -1)
            {
                ...
                {
                    //Mode = "TT";
                    //PreviewGR f = new PreviewGR(GRId,Mode);
                    //f.Show();
        ...
        //created by Thaddaeus Matthias , 25MAY2018, continue 28MAY2018
        //====================================begin====================================
